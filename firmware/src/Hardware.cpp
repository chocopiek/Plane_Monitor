...
void Hardware::loadConfig() {
  _prefs.begin("hw_config", true); // read-only
  _autoWaterThreshold = _prefs.getInt("aw_threshold", 30);
  _pumpDurationLimit  = _prefs.getInt("pump_limit", MAX_PUMP_DURATION_SEC);
  _prefs.end();
}

void Hardware::saveConfig() {
  _prefs.begin("hw_config", false);
  _prefs.putInt("aw_threshold", _autoWaterThreshold);
  _prefs.putInt("pump_limit", _pumpDurationLimit);
  _prefs.end();
}
...
void Hardware::update() {
  ...
  if (_lastAutoWaterMillis != 0 && now - _lastAutoWaterMillis < _pumpDurationLimit) return; // Cooldown at pump duration limit to prevent rapid re-triggering
